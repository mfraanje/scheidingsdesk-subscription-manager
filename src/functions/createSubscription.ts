...
async function createSubscription(
	...
	try {
		...
		context.info(
			`Recurring payment created with ID: ${recurringPaymentResponse.id}`,
		);
		...
		await addDataverseSubscription(
			customerId as string,
			recurringPaymentResponse.id,
			status,
			context,
		);
...
async function checkExistingSubscription(
	...
	try {
		const existingSubscriptions = await mollieClient.customerSubscriptions.list(
			{ customerId: customerId as string },
		);
